<template>
  <div class="h-screen flex justify-center items-center">
    <div
      class="hidden md:block lg:w-1/3 bg-white h-full auth-background rounded-tr-lg rounded-br-lg"
    />
    <div class="w-auto md:w-2/4 lg:w-2/3 flex justify-center items-center">
      <div class="w-full lg:w-1/2 px-10 lg:px-0">
        <h2 class="font-normal mb-6 text-2xl text-white">
          Sign In to Your Account
        </h2>
        <form method="post" @submit.prevent="login">
          <div class="mb-4">
            <label class="font-normal text-lg text-white block mb-2"
              >Email Address</label
            >
            <input
              v-model="email"
              type="email"
              class="auth-form focus:outline-none focus:bg-purple-hover focus:shadow-outline focus:border-purple-hover-stroke focus:text-gray-100"
              placeholder="Write your email address here"
            />
          </div>
          <div class="mb-4">
            <label class="font-normal text-lg text-white block mb-2"
              >Password</label
            >
            <input
              v-model="password"
              type="password"
              class="auth-form focus:outline-none focus:bg-purple-hover focus:shadow-outline focus:border-purple-hover-stroke focus:text-gray-100"
              placeholder="Write your password here"
            />
          </div>
          <div class="mb-4">
            <button
              class="block w-full bg-orange-action hover:bg-green-action text-white font-semibold px-6 py-2 text-md rounded-full"
              type="submit"
            >
              Sign In
            </button>
          </div>
        </form>
        <div class="text-center">
          <p class="text-white text-md">
            Don't have account?
            <nuxt-link to="/register" class="no-underline text-orange-action"
              >Sign Up </nuxt-link
            >.
          </p>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: 'LoginPage',
  layout: 'auth',
  data() {
    return {
      email: '',
      password: '',
    }
  },
  methods: {
    async login() {
      const payload = {
        email: this.email,
        password: this.password,
      }
      await this.$store.dispatch('auth/Login', payload)
      this.$router.push({ path: '/' })
    },
  },
}
</script>

<style scoped>
.auth-background {
  background-color: #3b41e3;
  background-image: url('/sign-in-background.jpg');
  background-position: center;
  background-size: cover;
}
</style>
